...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to delete the Cell Style. Error:" & @error & " Extended:" & @extended)

	; Check if the Cell style exists.
	$bExists = _LOCalc_CellStyleExists($oDoc, "NewCellStyle")
	If @error Then _ERROR($oDoc, "Failed to test for Cell Style existing in document. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "Now does a Cell style called ""NewCellStyle"" exist in the document? True/False: " & $bExists)
