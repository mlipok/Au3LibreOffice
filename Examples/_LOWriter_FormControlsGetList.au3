...
Func Example()
	...
	_LOWriter_FormControlInsert($oForm, $LOW_FORM_CONTROL_TYPE_TEXT_BOX, 500, 300, 2000, 3000, "AutoIt_Form_Control")
	If @error Then _ERROR($oDoc, "Failed to insert a form control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Create a Form Control
	_LOWriter_FormControlInsert($oForm, $LOW_FORM_CONTROL_TYPE_PUSH_BUTTON, 2500, 1300, 2000, 1000, "AutoIt_Form_Control1")
	If @error Then _ERROR($oDoc, "Failed to insert a form control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Create a Form Control
	_LOWriter_FormControlInsert($oForm, $LOW_FORM_CONTROL_TYPE_FILE_SELECTION, 1500, 4300, 4000, 2000, "AutoIt_Form_Control2")
	If @error Then _ERROR($oDoc, "Failed to insert a form control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve an array of controls.
	$avControls = _LOWriter_FormControlsGetList($oForm)
	...
	$avControls = _LOWriter_FormControlsGetList($oForm, BitOR($LOW_FORM_CONTROL_TYPE_PUSH_BUTTON, $LOW_FORM_CONTROL_TYPE_TEXT_BOX))
	If @error Then _ERROR($oDoc, "Failed to retrieve an array of controls. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	For $i = 0 To $iCount - 1
		...
		_LOWriter_FormControlDelete($avControls[$i][0])
		If @error Then _ERROR($oDoc, "Failed to delete the control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	Next
