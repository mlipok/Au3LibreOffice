...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to look for Shape name. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "Does this document contain a Shape named ""Shape 1""? True/ False. " & $bReturn)

	; Delete the Shape.
	_LOWriter_ShapeDelete($oDoc, $oShape)
	If @error Then _ERROR($oDoc, "Failed to delete Shape. Error:" & @error & " Extended:" & @extended)
