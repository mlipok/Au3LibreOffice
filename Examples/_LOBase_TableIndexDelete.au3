...
Func Example()
	...
	If @error Then Return _ERROR($oDoc, "Failed to add index to the Table. Error:" & @error & " Extended:" & @extended)

	; Retrieve a list of Indexes.
	$asIndexes = _LOBase_TableIndexesGetNames($oTable)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve an array of indexes for this Table. Error:" & @error & " Extended:" & @extended)

	For $i = 0 To @extended - 1
		$sIndexes &= $asIndexes[$i] & @CRLF
	Next

	MsgBox($MB_OK, "", "The Indexes contained in this table are:" & @CRLF & $sIndexes & @CRLF & @CRLF & "Press Ok to delete the ""Item_Index"".")

	; Delete the Index "Item_Index".
	_LOBase_TableIndexDelete($oTable, "Item_Index")
	If @error Then Return _ERROR($oDoc, "Failed to delete index. Error:" & @error & " Extended:" & @extended)
	...
	MsgBox($MB_OK, "", "The Indexes contained in this table now are:" & @CRLF & $sIndexes)
