...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to retrieve a list of Shape names. Error:" & @error & " Extended:" & @extended)
	...
	If (UBound($asShapes) > 0) Then
		...
		If @error Then _ERROR($oDoc, "Failed to retrieve a Shape Object. Error:" & @error & " Extended:" & @extended)

		MsgBox($MB_OK, "", "Press ok to delete the Shape.")

		; Delete the Shape.
		_LOWriter_ShapeDelete($oDoc, $oShape)
		If @error Then _ERROR($oDoc, "Failed to delete a Shape. Error:" & @error & " Extended:" & @extended)

	Else
		_ERROR($oDoc, "Something went wrong, and no Shapes were found.")
	EndIf
