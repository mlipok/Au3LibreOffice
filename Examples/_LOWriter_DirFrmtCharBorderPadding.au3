...
Func Example()
	...
	_LOWriter_DirFrmtCharBorderPadding($oViewCursor, $i100thMM)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The selected text's current Character Border padding settings are as follows: " & @CRLF & _
			"All Padding distance, in Hundredths of a Millimeter (100th MM): " & $avSettings[0] & " This setting is best only used to set the distance, as" & _
	...
	_LOWriter_DirFrmtCharBorderPadding($oViewCursor, Null, $i100thMM, $i100thMM2, $i100thMM2, $i100thMM)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
