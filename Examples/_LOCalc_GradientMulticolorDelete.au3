...
Func Example()
	...
	_LOCalc_GradientMulticolorDelete($avStops, 0)
	If @error Then _ERROR($oDoc, "Failed to delete ColorStop. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Delete the first ColorStop
	_LOCalc_GradientMulticolorDelete($avStops, UBound($avStops) - 1)
	If @error Then _ERROR($oDoc, "Failed to delete ColorStop. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
