...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to retrieve next Redo action title. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "The next Redo action title is: " & $sRedo & @CRLF & " Press ok to perform it.")

	; Perform one Redo action.
	_LOCalc_DocRedo($oDoc)
	If @error Then _ERROR($oDoc, "Failed to perform an Redo action. Error:" & @error & " Extended:" & @extended)
