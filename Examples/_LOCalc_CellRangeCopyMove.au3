...
Func Example()
	...
	$oCellRange = _LOCalc_SheetGetCellByName($oSheet, "B2", "D4")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Range Object. Error:" & @error & " Extended:" & @extended)

	; Retrieve Cell D6
	$oCell = _LOCalc_SheetGetCellByName($oSheet, "D6")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Object. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "I will move the cell Range B2-D4 to begin at cell D6")

	; Copy the Cell Range to Cell D6
	_LOCalc_CellRangeCopyMove($oSheet, $oCellRange, $oCell, False)
	If @error Then _ERROR($oDoc, "Failed to move Cell Range. Error:" & @error & " Extended:" & @extended)
	...
	$oCell = _LOCalc_SheetGetCellByName($oSheet2, "C7")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Object. Error:" & @error & " Extended:" & @extended)
	...
	_LOCalc_CellRangeCopyMove($oSheet, $oCellRange, $oCell, True)
	If @error Then _ERROR($oDoc, "Failed to move Cell Range. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR($oDoc, "Failed to switch active sheets. Error:" & @error & " Extended:" & @extended)
