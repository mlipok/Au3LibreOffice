...
Func Example()
	...
	If @error Then Return _ERROR($oDoc, "Failed to modify index settings. Error:" & @error & " Extended:" & @extended)

	; Retrieve an array of settings for the Index.
	$avSettings = _LOBase_TableIndexModify($oTable, "Item_Index")
	If @error Then Return _ERROR($oDoc, "Failed to retrieve current index settings. Error:" & @error & " Extended:" & @extended)
	...
	MsgBox($MB_OK, "", "The Index, ""Item_Index"" has the following settings: " & @CRLF & _
