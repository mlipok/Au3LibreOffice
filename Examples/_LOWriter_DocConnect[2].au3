...
Func Example()
	...
	If (@error > 0) Or Not IsObj($oDoc2) Then _ERROR($oDoc, "Failed to Connect to Writer Document. Error:" & @error & " Extended:" & @extended)

	; Retrieve Doc Name.
	$sDocName = _LOWriter_DocGetName($oDoc, False)
	If @error Then _ERROR($oDoc, "Failed to retrieve Writer Document name. Error:" & @error & " Extended:" & @extended)
