...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to create a Text Cursor in the Comment. Error:" & @error & " Extended:" & @extended)

	; Insert some Text.
	_LOCalc_TextCursorInsertString($oTextCursor, "Plus some added text.")
	If @error Then _ERROR($oDoc, "Failed to insert a string into the comment. Error:" & @error & " Extended:" & @extended)

	; Select the new text.
	_LOCalc_TextCursorMove($oTextCursor, $LOC_TEXTCUR_GO_LEFT, 21, True)
	If @error Then _ERROR($oDoc, "Failed to select text using the Text Cursor. Error:" & @error & " Extended:" & @extended)

	; Set the text to Bold.
	_LOCalc_TextCursorFont($oDoc, $oTextCursor, Null, Null, Null, $LOC_WEIGHT_BOLD)
	If @error Then _ERROR($oDoc, "Failed to set Text Cursor font settings. Error:" & @error & " Extended:" & @extended)
