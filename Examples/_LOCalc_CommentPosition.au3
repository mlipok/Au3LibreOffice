...
Func Example()
	...
	; Convert 4 Inch to Hundredths of a Millimeter (100th MM)
	$i100thMM1 = _LO_UnitConvert(4, $LO_CONVERT_UNIT_INCH_100THMM)
	If @error Then _ERROR($oDoc, "Failed to convert Hundredths of a Millimeter (100th MM). Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Convert 1.5 Inches to Hundredths of a Millimeter (100th MM)
	$i100thMM2 = _LO_UnitConvert(1.5, $LO_CONVERT_UNIT_INCH_100THMM)
	If @error Then _ERROR($oDoc, "Failed to convert Hundredths of a Millimeter (100th MM). Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Modify the Comment's Position to: 4 Inches Right, and 1.5 inches down from the Cell B3. And lock the position.
	_LOCalc_CommentPosition($oComment, $i100thMM1, $i100thMM2, True)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Comment's Position settings are as follows: " & @CRLF & _
			"The Horizontal distance from the insertion point is, in Hundredths of a Millimeter (100th MM): " & $avSettings[0] & @CRLF & _
			"The Vertical distance from the insertion point is, in Hundredths of a Millimeter (100th MM): " & $avSettings[1] & @CRLF & _
