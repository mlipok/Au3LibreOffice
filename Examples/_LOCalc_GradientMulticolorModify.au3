...
Func Example()
	...
	_LOCalc_GradientMulticolorModify($avStops, 0, 0.1, 123)
	If @error Then _ERROR($oDoc, "Failed to modify ColorStop. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Modify the first ColorStop
	_LOCalc_GradientMulticolorModify($avStops, UBound($avStops) - 1, 0.8, 1655334)
	If @error Then _ERROR($oDoc, "Failed to modify ColorStop. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
