...
Func Example()
	...
	If (@error > 0) Or Not IsArray($aPrinters) Then _ERROR($oDoc, "Error retrieving Array of Printers. Error:" & @error & " Extended:" & @extended)

	If ($aPrinters[0] <= 1) Then _ERROR($oDoc, "one or no Printers found. Error:" & @error & " Extended:" & @extended)
