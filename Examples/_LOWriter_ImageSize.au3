...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Image's sizes are as follows: " & @CRLF & _
			...
			"The Image's width, in Hundredths of a Millimeter (100th MM), is: " & $avSettings[2] & @CRLF & _
			"The Image's height, in Hundredths of a Millimeter (100th MM), is: " & $avSettings[3] & @CRLF & _
	...
	; Convert 7" to Hundredths of a Millimeter (100th MM)
	$i100thMM2 = _LO_UnitConvert(7, $LO_CONVERT_UNIT_INCH_100THMM)
	If @error Then _ERROR($oDoc, "Failed to convert from inches to Hundredths of a Millimeter (100th MM). Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Set the Image's size. Skip Scale Width, Skip Scale Height, Set Width to 4", height to 7"
	_LOWriter_ImageSize($oImage, Null, Null, $i100thMM, $i100thMM2)
	If @error Then _ERROR($oDoc, "Failed to set Image settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
