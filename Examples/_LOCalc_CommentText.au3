...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to set Comment text. Error:" & @error & " Extended:" & @extended)

	; Retrieve the Comment's current Text.
	$sComment = _LOCalc_CommentText($oComment)
	If @error Then _ERROR($oDoc, "Failed to retrieve Comment Text. Error:" & @error & " Extended:" & @extended)
