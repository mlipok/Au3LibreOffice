...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to set comment Area Color. Error:" & @error & " Extended:" & @extended)

	; Set the Comment's Transparency to 40%
	_LOCalc_CommentAreaTransparency($oComment, 40)
	If @error Then _ERROR($oDoc, "Failed to set comment Area Transparency. Error:" & @error & " Extended:" & @extended)

	; Retrieve the current Transparency value
	$iTransparency = _LOCalc_CommentAreaTransparency($oComment)
	If @error Then _ERROR($oDoc, "Failed to retrieve comment Area Transparency. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "The Comment's background color transparency percentage is: " & $iTransparency)
