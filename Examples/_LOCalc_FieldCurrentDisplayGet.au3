...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to retrieve field's current display. Error:" & @error & " Extended:" & @extended)

	; Retrieve the field's command name.
	$sCommand = _LOCalc_FieldCurrentDisplayGet($mField, True)
	If @error Then _ERROR($oDoc, "Failed to retrieve field's current display. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "The Field's current display is: " & $sDisplay & @CRLF & _
