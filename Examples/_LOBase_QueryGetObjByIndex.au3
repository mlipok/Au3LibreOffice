...
Func Example()
	...
	If @error Then Return _ERROR($oDoc, "Failed to retrieve Query Object. Error:" & @error & " Extended:" & @extended)

	; Retrieve the Query's SQL command
	$sSQL = _LOBase_QuerySQLCommand($oQuery)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve Query SQL command. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "This Query's SQL command is: " & $sSQL)
