...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to insert field at Text Cursor 4. Error:" & @error & " Extended:" & @extended)

	;Insert a space after the field.
	_LOCalc_TextCursorInsertString($oTextCursor, " ")
	If @error Then _ERROR($oDoc, "Failed to insert a String. Error:" & @error & " Extended:" & @extended)

	; Insert a Sheet name field in the cell A1.
	_LOCalc_FieldSheetNameInsert($oDoc, $oTextCursor)
	If @error Then _ERROR($oDoc, "Failed to insert field at Text Cursor 2. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR($oDoc, "Failed to insert field at Text Cursor 3. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR($oDoc, "Failed to insert field at Text Cursor 1. Error:" & @error & " Extended:" & @extended)

	; Retrieve an array of all Field Objects.
	$avFields = _LOCalc_FieldsGetList($oTextCursor)
	If @error Then _ERROR($oDoc, "Failed to retrieve array of fields. Error:" & @error & " Extended:" & @extended)
