...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to modify Footnote settings. Error:" & @error & " Extended:" & @extended)

	; Retrieve the current Footnote Label.
	$sLabel = _LOWriter_FootnoteModifyAnchor($oFootNote)
	If @error Then _ERROR($oDoc, "Failed to Retrieve Footnote settings. Error:" & @error & " Extended:" & @extended)
