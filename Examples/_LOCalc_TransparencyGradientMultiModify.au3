...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Comment's Transparency Gradient ColorStops are as follows: " & @CRLF & _
			$sStops & @CRLF & @CRLF & _
			"Press ok to modify the first and last ColorStop.")

	; Modify the first ColorStop.
	_LOCalc_TransparencyGradientMultiModify($avStops, 0, 0.1, 76)
	...
	_LOCalc_TransparencyGradientMultiModify($avStops, UBound($avStops) - 1, 0.8, 14)
	If @error Then _ERROR($oDoc, "Failed to modify a ColorStop. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
