...
Func Example()
	...
	_LOBase_ReportOpen($oDoc, $oConnection, "rptReport1", True, True)
	If @error Then Return _ERROR($oDoc, "Failed to open a Report Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "I have opened the Report called ""rptReport1"" in Design Mode. Press ok to connect to it, and close it.")

	; Connect to the open Report.
	$oReportDoc = _LOBase_ReportConnect(True)
