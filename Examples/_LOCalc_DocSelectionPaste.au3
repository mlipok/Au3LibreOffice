...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to activate a new Sheet. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	If @error Then _ERROR($oDoc, "Failed to Create a new Calc Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber, $oNewDoc)

	; Retrieve the active Sheet.
	$oSheet = _LOCalc_SheetGetActive($oNewDoc)
	If @error Then _ERROR($oDoc, "Failed to retrieve the currently active Sheet Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber, $oNewDoc)

	; Retrieve the Cell D5 in the new Document.
	$oCell = _LOCalc_RangeGetCellByName($oSheet, "D5")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber, $oNewDoc)

	; Select the Cell.
	_LOCalc_DocSelectionSet($oNewDoc, $oCell)
	If @error Then _ERROR($oDoc, "Failed to set Selection. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber, $oNewDoc)
	...
	If @error Then _ERROR($oDoc, "Failed to close opened L.O. Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber, $oNewDoc)
