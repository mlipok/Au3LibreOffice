...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to retrieve a list of Images. Error:" & @error & " Extended:" & @extended)
	...
	If (UBound($asImages) > 0) Then
		...
		If @error Then _ERROR($oDoc, "Failed to retrieve an Image Object. Error:" & @error & " Extended:" & @extended)

		MsgBox($MB_OK, "", "Press ok to delete the Text Image.")

		; Delete the Image.
		_LOWriter_ImageDelete($oDoc, $oImage)
		If @error Then _ERROR($oDoc, "Failed to delete an Image. Error:" & @error & " Extended:" & @extended)

	Else
		_ERROR($oDoc, "Something went wrong, and no Images were found.")
	EndIf
