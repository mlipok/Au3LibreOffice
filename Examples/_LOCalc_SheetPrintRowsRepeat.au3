...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to set Row Heading Range. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "I have set Row 1 to repeat automatically on every printed page to the bottom.")

	; Retrieve the current print ranges set for this sheet.
	$avSettings = _LOCalc_SheetPrintRowsRepeat($oSheet)
	If @error Then _ERROR($oDoc, "Failed to retrieve Row repeat Range. Error:" & @error & " Extended:" & @extended)
	...
	MsgBox($MB_OK, "", "The Range(s) currently set to be repeated as Row Headers are: " & @CRLF & $sRanges & @CRLF & @CRLF & _
	...
	If @error Then _ERROR($oDoc, "Failed to set Repeat Row Range. Error:" & @error & " Extended:" & @extended)

	; Retrieve the current print ranges set for this sheet.
	$avSettings = _LOCalc_SheetPrintRowsRepeat($oSheet)
	If @error Then _ERROR($oDoc, "Failed to retrieve Row Repeat Range array. Error:" & @error & " Extended:" & @extended)
	...
	MsgBox($MB_OK, "", "The Range(s) currently set to be repeated as Row Headers are: " & @CRLF & $sRanges)
