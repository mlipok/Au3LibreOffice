...
Func Example()
	...
	If @error Then _ERROR($oDoc, "Failed to retrieve E5 Cell Object. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR($oDoc, "Failed to Set E5 Cell content. Error:" & @error & " Extended:" & @extended)

	; Retrieve the C5 Cell.
	$oCell = _LOCalc_RangeGetCellByName($oSheet, "C5")
	If @error Then _ERROR($oDoc, "Failed to retrieve C5 Cell Object. Error:" & @error & " Extended:" & @extended)

	; Set C5 Cell Formula to A1 + A2
	_LOCalc_CellFormula($oCell, "=A1+A2")
	If @error Then _ERROR($oDoc, "Failed to Set C5 Cell content. Error:" & @error & " Extended:" & @extended)

	; Retrieve the A7 Cell.
	$oCell = _LOCalc_RangeGetCellByName($oSheet, "A7")
	If @error Then _ERROR($oDoc, "Failed to retrieve A7 Cell Object. Error:" & @error & " Extended:" & @extended)

	; Set A7 Cell Formula to A7 - C3
	_LOCalc_CellFormula($oCell, "=C5 - C3")
	If @error Then _ERROR($oDoc, "Failed to Set A7 Cell content. Error:" & @error & " Extended:" & @extended)

	; Mark all levels of precedents for Cell A7
	While _LOCalc_SheetDetectivePrecedent($oCell)
		If @error Then _ERROR($oDoc, "Failed to mark Cell precedents. Error:" & @error & " Extended:" & @extended)
	WEnd
	...
	If @error Then _ERROR($oDoc, "Failed to clear Detective markings. Error:" & @error & " Extended:" & @extended)
