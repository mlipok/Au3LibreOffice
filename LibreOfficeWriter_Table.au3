...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: LibreOffice counts columns and Rows starting at 0. So to delete the first column in a Table you would set $iColumn to 0. If you attempt to delete more columns than are present all columns from $iColumn over will be deleted. If you delete all columns starting from column 0, the entire Table is deleted.
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: If you do not set $iColumn, the new columns will be placed at the end of the Table. Note, LibreOffice counts the Table columns/Rows starting at 0. The columns are placed behind the desired column when inserted. To insert a column at the left most of the Table you would set $iColumn to 0. To insert columns at the Right of a table you would set $iColumn to one higher than the last column. e.g. a Table containing 3 columns, would be numbered as follows: 0(first-Column), 1(second-Column), 2(third-Column), to insert columns at the very Right of the columns, you would set $iColumn to 3.
; Related .......: _LOWriter_TableInsert, _LOWriter_TableGetObjByCursor, _LOWriter_TableGetObjByName, _LOWriter_TableColumnGetCount
; Link ..........:
; Example .......: Yes
; ===============================================================================================================================
...
;				   Call any optional parameter with Null keyword to skip it.
;				   The TableName may change upon inserting it into the document if there is a table already named the same, (e.g. TableName becomes TableName1).
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: This function will accept a Table Cursor, a ViewCursor, or a Text Cursor. A TableCursor and ViewCursor can retrieve the single cell they are located in, or a range of cells that have been selected by them. A TextCursor can only retrieve the single cell it is located in.
...
;				   Rows and Columns in a Table are 0 based, meaning they start their count at 0. the first cell is row 0 column 0. To retrieve a single cell, only call the $iColumn and $iRow parameters. To retrieve a cell range, call $iColumn with the lowest integer value column and then $iToColumn with the highest integer value column desired. Same for $iRow and $iToRow. You may request the same row in both $iRow and $iToRow, but neither $iToRow or $iToColumn may be a lower integer value than $iRow and $iColumn respectively.
...
;				   TextTables cannot be inserted into Foot/Endnotes. And it is not best to place them into other tables, though it is possible. You can set the $oCursor parameter  to either a ViewCursor or a Text cursor currently in an acceptable data type, the table will be inserted at the cursor position. Or set $oCursor to Default, the Table will be inserted at the very end of a document.
;				   $bHeading only applies when a Table is created with more than 1 Row. For an unknown reason to myself, when creating a TextTable with more than 1 row the "Table Heading" paragraph style is applied to the first row. Setting $bHeading to False (Default) will return the paragraph style to "Table Contents" as normal. If you set $bHeading to True, "Table Heading" Paragraph Style will be applied. If these styles are not present a Property setting error will result, however the Table will still have been successfully inserted into the document.
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: LibreOffice counts Rows starting at 0. So to delete the first Row in a Table you would set $iRow to 0. If you attempt to delete more rows than are present, all rows from $iRow over will be deleted.
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: If you do not set $iRow, the new Rows will be placed at the Bottom of the Table. Note, LibreOffice counts the Table Rows starting at 0. The Rows are placed above the desired Row when inserted. To insert a Row at the top most of the Table you would set $iRow to 0. To insert rows at the bottom of a table you would set $iRow to one higher than the last row. e.g. a Table containing 3 rows, would be numbered as follows: 0(first-row), 1(second-row), 2(third-row), to insert rows at the very bottom of the rows, I would set $iRow to 3.
; Related .......: _LOWriter_TableInsert, _LOWriter_TableGetObjByCursor, _LOWriter_TableGetObjByName, _LOWriter_TableRowGetCount
; Link ..........:
; Example .......: Yes
; ===============================================================================================================================
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: The array must be an array of Arrays. The Main Array must contain the same number of elements as there are rows, and each sub Array must have the same number of Elements as there are columns. To skip a Cell, just leave the sub array element blank you want to skip.
;				   This will replace all previous data in the Table. The Array will not be modified.
; Related .......: _LOWriter_TableInsert, _LOWriter_TableGetObjByCursor, _LOWriter_TableGetObjByName
; Link ..........:
; Example .......: Yes
; ===============================================================================================================================
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: Relative Width and Width cannot be set until the Table Horizontal orientation is set to other than $LOW_ORIENT_HORI_FULL(6), which is LibeOffice's default setting. Note: Width may change +/- 1 Micrometer once set due to Libre Office.
;				   Call this function with only the required parameters (or with all other parameters set to Null keyword), to get the current settings.
;				   Call any optional parameter with Null keyword to skip it.
; Related .......: _LOWriter_TableInsert, _LOWriter_TableGetObjByCursor, _LOWriter_TableGetObjByName, _LOWriter_ConvertFromMicrometer, _LOWriter_ConvertToMicrometer
; Link ..........:
