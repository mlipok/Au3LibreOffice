...
; Remarks .......: Call this function with only the required parameters (or with all other parameters set to Null keyword), to get the current settings.
;                  Call any optional parameter with Null keyword to skip it.
;                  LibreOffice may change the shadow distance +/- a Hundredth of a Millimeter (100th MM).
...
;                  $iSpacing            - [optional] an integer value (0-240005). Default is Null. The amount of space between the Callout connector line end and the comment box, in Hundredths of a Millimeter (100th MM).
...
;                  If $iExtension is set to $LOC_COMMENT_CALLOUT_EXT_OPTIMAL, $LOC_COMMENT_CALLOUT_EXT_FROM_LEFT, or $LOC_COMMENT_CALLOUT_EXT_FROM_TOP, $iExtendBy will be set to the length to extend the Callout line from the Comment box, in Hundredths of a Millimeter (100th MM).
...
; Parameters ....: $oComment            - [in/out] an object. A Comment object returned by a previous _LOCalc_CommentsGetList, _LOCalc_CommentGetObjByCell, or _LOCalc_CommentGetObjByIndex function.
;                  $iWidth              - [optional] an integer value. Default is Null. The width of the Comment, in Hundredths of a Millimeter (100th MM). Min. 51.
;                  $iHeight             - [optional] an integer value. Default is Null. The height of the Comment, in Hundredths of a Millimeter (100th MM). Min. 51.
...
;                  $iIncrement          - [optional] an integer value (0-25400). Default is Null. The increment value measured in Hundredths of a Millimeter (100th MM). See remarks.
...
; Author ........: donnyh13
; Modified ......:
; Remarks .......: The Increment value can be set in either Pixels or Hundredths of a Millimeter (100th MM), but internally it uses the same property. To make setting this easier, I have made two separate parameters to set, depending on if you wish to set increment using pixels ($iPixelIncrement) or Hundredths of a Millimeter ($iIncrement). Only one should be used, otherwise $iIncrement will overwrite $iPixelIncrement.
...
;                  $iSpacing            - [optional] an integer value. Default is Null. The amount of spacing between the columns, in Hundredths of a Millimeter (100th MM).
; Return values .: Success: 1 or Array
;                  Failure: 0 and sets the @Error and @Extended flags to non-zero.
;                  --Input Errors--
;                  @Error 1 @Extended 1 Return 0 = $oDoc not an Object.
